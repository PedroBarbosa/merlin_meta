 ...
 *
 */
package alignment.blast.org.biojava3.ws.alignment.qblast;
...
import alignment.blast.org.biojava3.ws.alignment.RemotePairwiseAlignmentOutputProperties;
import alignment.blast.org.biojava3.ws.alignment.RemotePairwiseAlignmentProperties;
...
public class NcbiBlastService implements RemotePairwiseAlignmentService {
	 ...
	 * @throws Exception 
	 */
	public NcbiBlastService(int timeout, Project project) throws Exception {
		...
		try {
			...
			if(project.isUseProxy()) {
				...
				System.setProperty("http.proxyHost", project.getProxy_host());
				System.setProperty("http.proxyPort", project.getProxy_port());
				//this.proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress(project.getProxy_host(), Integer.parseInt(project.getProxy_port())));

				if(project.isUseAuthentication()) {

					System.setProperty("http.proxyUser", project.getProxy_username());
					System.setProperty("http.proxyPassword", project.getProxy_password());
				}
			}
	...
	private String sendActualAlignementRequest(String str,
		...
		cmd = rpa.getBlastCommandsToQBlast() 
				//				+ "&TOOL=" + this.getTool()
				//				+ "&EMAIL=" + this.getEmail()
				;
		 ...
		 * DEBUG LINE
		 */
		//System.out.println(baseurl+this.cmd);
		String line = "";
	...
	public String sendAlignmentRequest(String str,
		...
		while(rid.length()>12 || rid.length()<10 || !isAllUpper(rid) || rid.contains(" ") || rid.contains("_") || rid == null) {
			...
			if(counter>50) {
				...
				//System.err.println("New rid error. New rid = \t"+rid);
				throw new Exception("Cannot get RID for alignment");
			}
			//System.out.println("rid id error asking for new rid\t"+rid);
			 ...
			 */
				...
				try {
					...
					while ((line = rd.readLine()) != null) {
						...
						//									"<tr><td>Request ID</td><td> <b>", "").replace("" +
						//											"</b></td></tr>", ""));
						//						}
