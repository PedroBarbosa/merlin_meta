					...
					if(aRid!=null) {
						...
						if(timeSinceDeployment<this.latencyWaitingPeriod) {
							...
							if(currentRequestTimer - lastRequestTimer > 60000) {
								...
								//requestReady = this.rbw.isReady(aRid, System.currentTimeMillis());
							}
									...
									if(blastToList.isSimilarityFound()) {
										...
										if(!this.cancel.get()) {
											...
											if(isNCBIGenome) {
												...
												homologyDataNcbiClient = new HomologyDataClient(blastToList, this.taxonomyMap, this.uniprotStar,this.cancel, HomologySearchServer.NCBI, this.rqb.getHitlistSize(), this.uniprotStatus);
											}
											else {

												homologyDataNcbiClient = new HomologyDataClient(blastToList, this.orgArray, this.taxonomyMap, this.uniprotStar,this.cancel, HomologySearchServer.NCBI, this.rqb.getHitlistSize(), this.uniprotStatus);
											}
											
											//System.out.println("Retrieved for"+ homologyData.isDataRetrieved());

											if(homologyDataNcbiClient.isDataRetrieved()) {
												
												//System.out.println("\tRetrieved for "+blastToList.getQuery());
										...
										if(!this.cancel.get()) {
											...
											HomologyDataClient homologyDataNcbiClient = new HomologyDataClient(this.queryRIDMap.get(aRid).split("%0D")[0].replace("%3E", ""),this.rqb.getBlastProgram(),this.cancel, this.uniprotStatus,HomologySearchServer.NCBI);
